...
const tableNameGroupon = 'groupon'
...
router.get("/cartlist", async (req, res) => {
    ...
    let data1 = await Db.select(req, `SELECT a.*,b.goodsname,b.price,b.img FROM ${tableName} a LEFT JOIN ${tableNameGroupon} b ON a.goodsid = b.id WHERE a.uid = '${uid}' AND a.type=2`);
    let data2 = await Db.select(req, `SELECT a.*,b.goodsname,b.price,b.img FROM ${tableName} a LEFT JOIN ${tableNameGoods} b ON a.goodsid = b.id WHERE a.uid = '${uid}' AND type=1`);
    let data = [];
...
router.post("/cartadd", async (req, res) => {
    let { uid,type,goodsid,num } = req['body'];
    const info = await Db.select(req, `SELECT * FROM ${tableName} WHERE uid = '${uid}' AND type= ${type} AND goodsid = ${goodsid} `);
    let result = '';
    if (info) {
        //更新数量
